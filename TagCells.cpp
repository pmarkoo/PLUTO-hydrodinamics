...
/* ----------------------------------------------------------------
  ...
  /* -----------------------------------------------
    ...
    if ( D_EXPAND((i<=1), && (j<=1), && (k<=1)) )  grad[k][j][i] = 1.0;  //always refine the 2 inermost cells
    //double radius = sqrt(x1*x1+x2*x2+x3*x3);
    //if (radius <= 2.0) grad[k][j][i] = 1.0;
   
    //  Part making the maxLevel non-constant, deactivated now to 
    //  provide the simplest configuration

    int new_maxLevel;
    int maxLevel = 0;
    maxLevel = atoi(ParamFileGet("Levels",1));

    //new_maxLevel = (int) (maxLevel + 1.0 - 3.0*(g_time/10.0));

    new_maxLevel = (int) log2(pow(2,maxLevel+1)/(g_rshock/0.5));

	
    if ((m_level >= new_maxLevel) || (UU[RHO][k][j][i] < g_inputParam[RHOamb])) grad[k][j][i] = 0.0; 
 ...
 *********************************************************************** */
{
  ...
  BOX_LOOP(Ubox, k, j, i) 
  {
    ...
    q[k][j][i] = us[PRS]/us[RHO]; //proportional to temperature
  }
